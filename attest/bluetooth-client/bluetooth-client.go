...
func main() {
	...
	// At this point we should verify that the EK certificate chains back
	// to a known TPM manufacturer, but that's not needed for this PoC.
	...
	// We now have an AK and an EK. Verify that the AK corresponds
	// to the EK.
	...
	// want a quote and a copy of the event log.
	...
	// Verify that the event log matches the PCR values we got
	events, err := el.Verify(att.PCRs)
	if err != nil {
		log.Fatalf("Log failed to replay: %v", err)
	}

	// Finally, check the secure boot state in the event log
